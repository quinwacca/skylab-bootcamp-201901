...
const {
    env: { TOKEN_KEYWORD, DB_URL, SPOTIFY_API_TOKEN }
} = process;
...
describe("logic", () => {
    ...
    describe("authenticate user", () => {
        ...
        it("should succeed on correct credentials", () =>
            logic.authenticateUser(email, password).then(({ id, token }) => {
                ...
                expect(jwt.verify(token, TOKEN_KEYWORD).id).toEqual(id);
            }));
    });
        ...
        beforeEach(() =>
            users
                ...
                .then(user => {
                    ...
                    _token = jwt.sign({ _id }, TOKEN_KEYWORD);
                })
        );
    ...
    describe("update user", () => {
        ...
        it("should succeed on correct update", () =>
            logic
                .updateUser(_id, _token, dataToBeUpdated)
                .then(({ value: { _id } }) => users.findById(_id))
                .then(user => {
                    expect(user._id.toString()).toBe(_id);
                    expect(user.name).toBe(dataToBeUpdated.name);
                    expect(user.surname).toBe(dataToBeUpdated.surname);
                    expect(user.email).toBe(email);
    ...
    describe("list comments from artist", () => {
        ...
        beforeEach(() =>
            ...
            userApi
                ...
                .then(() =>
                    artistComments.add(
                        (comment = { userId: _id, artistId, text })
                    )
                )
                .then(() =>
                    artistComments.add(
        ...
        it("should succeed on correct data", () =>
            logic.listCommentsFromArtist(artistId).then(comments => {
                ...
                comments.forEach(
                    ({ id, userId, artistId: _artistId, date }) => {
                        expect(id).toBeDefined();
